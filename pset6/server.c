...
char* indexes(const char* path)
{
    ...
    int path_index = strlen(path);
    

    char* php_index = calloc(1, (path_index + 11) * sizeof(char));
    ...
    strcpy(php_index, path);
    strcat(php_index, "/index.php");
    
    if (access(php_index, F_OK) == -1)
        free(php_index);
    else 
        return php_index;
    
    char* html_index = calloc(1, (path_index + 12) * sizeof(char));
    ...
    strcpy(html_index, path);
    strcat(html_index, "/index.html");
    
    if (access(html_index, F_OK) == -1)
    {
        free(html_index);
        return NULL;
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    for (c = fgetc(file); c != EOF; c = fgetc(file))
    {
        ...
        if (count + 1 > buff_cap)
        {
            ...
            char* buff_temp = realloc(buffer, buff_cap * sizeof(char));
            if (buff_temp == NULL)
            {
                free(buffer);
                return false;
    ...
    char* storage = malloc((count) * sizeof(char));
    memcpy(storage, buffer, count);
    free(buffer);
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char parse_tmp[((strlen(line) + 1) * sizeof(char))];
    strcpy(parse_tmp, line);
    
    //create storage space for method, target and version strings
    char method[((strlen(parse_tmp)+1) * sizeof(char))];
    char target[((strlen(parse_tmp)+1) * sizeof(char))];
    char version[((strlen(parse_tmp)+1) *sizeof(char))];
    ...
    //check to see if target contains a ?
    if (strchr(target, '?') == NULL)
    {
    ...
    {
        token_counter = 0;
        char* qtoken = strtok(target, "?");
        
        while (qtoken != NULL)
        {
            if (token_counter == 0)
            {
                strcpy(abs_path, qtoken);
            }
            else if(token_counter == 1)
            {
                strcpy(query, qtoken);
